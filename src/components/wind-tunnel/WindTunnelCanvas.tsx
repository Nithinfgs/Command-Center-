...
export const WindTunnelCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const getFlowVelocityAt = (wx: number, wy: number): { u: number; v: number; speedNorm: number; tempK: number } => {
        const u = 1.0;
        ...
        if (xb < noseTipOffset) {
          ...
          if (distToNose < noseInfluenceRadius) {
            ...
            tempK = 288 + (aero.stagnationTemperature - 288) * frac;
      ...
      const streamlines: { points: { x: number; y: number; speedNorm: number; tempK: number }[] }[] = [];

      for (let s = 0; s < NUM_STREAMLINES; s++) {
        let currX = 0;
        let currY = rakeTop + s * rakeSpacing;
        const pts: { x: number; y: number; speedNorm: number; tempK: number }[] = [];
      ...
      for (let s = 0; s < streamlines.length; s++) {
        ...
        for (let p = 0; p < pts.length - 1; p++) {
          ...
          ctx.strokeStyle = windTunnelState.visualizationMode === 'thermal'
            ? getCfdColor(Math.min(1, (pt1.tempK - 273) / Math.max(100, aero.stagnationTemperature - 273)), 0.82)
            : getCfdColor(pt1.speedNorm, 0.88);
      ...
      for (const part of vehicleGeometry.parts) {
        ...
        if (def.texturePattern === 'cone') {
          ...
          const glowGrad = ctx.createRadialGradient(0, -ph / 2 - 4, 1, 0, -ph / 2 - 4, 30);
          glowGrad.addColorStop(0, '#ffffff');
          glowGrad.addColorStop(0.35, '#ffb703');
          glowGrad.addColorStop(0.75, 'rgba(239, 68, 68, 0.7)');
          glowGrad.addColorStop(1, 'rgba(239, 68, 68, 0)');
          ctx.fillStyle = glowGrad;
          ctx.beginPath();
          ctx.arc(0, -ph / 2 - 4, 30, 0, Math.PI * 2);
          ctx.fill();
        } else if (def.texturePattern === 'engine-bell') {
  ...
  }, [blueprint, windTunnelState, aero, plume, vehicleGeometry, rocketProps]);

  return (
    <div className="relative flex-1 h-full bg-[#03060f] overflow-hidden select-none">
      ...
      <div className="absolute top-3 right-3 bg-[#090f1d]/95 border-2 border-[#00e5ff]/60 rounded-xl p-3.5 text-xs font-mono shadow-2xl w-80 space-y-2.5 backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]">
          ...
          <div className="bg-[#03060f] p-2 rounded-lg border border-[#1a2638]">
            ...
            <div className="text-[#ffb703] font-bold text-sm mt-0.5">{aero.stagnationTemperature} K</div>
          </div>
          <div className="bg-[#03060f] p-2 rounded-lg border border-[#1a2638]">
      ...
      <div className="absolute top-3 left-3 bg-[#090f1d]/90 border border-[#1e2d42] px-3 py-2 rounded-lg text-[10px] font-mono text-slate-300 shadow-xl space-y-1">
        <div className="font-bold text-slate-200 uppercase tracking-wider">CFD VELOCITY & PRESSURE MAP</div>
        <div className="w-36 h-2.5 rounded-full overflow-hidden bg-gradient-to-r from-blue-600 via-cyan-400 via-emerald-400 via-amber-400 to-red-600 shadow-inner" />
